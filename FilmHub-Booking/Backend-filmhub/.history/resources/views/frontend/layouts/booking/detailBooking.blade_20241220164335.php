                            ...
                            <div class="row">
                                                    ...
                                                    <select id="combo" name="combo_id" class="form-control"
                                                        ...
                                                        @foreach ($combos as $combo)
                                                            <option value="{{ $combo->id }}"
                                                                ...
                                                                {{ $combo->name }} - {{ number_format($combo->price) }}
                                                                VNĐ
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    <select id="food" name="food_id" class="form-control"
                                                        ...
                                                        @foreach ($foods as $food)
                                                            <option value="{{ $food->id }}"
                                                                ...
                                                                {{ $food->name }} - {{ number_format($food->price) }}
                                                                VNĐ
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    <select id="drink" name="drink_id" class="form-control"
                                                        ...
                                                        @foreach ($drinks as $drink)
                                                            <option value="{{ $drink->id }}"
                                                                ...
                                                                {{ $drink->name }} - {{ number_format($drink->price) }}
                                                                VNĐ
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                            ...
                                                            @foreach ($vouchers as $voucher)
                                                                @if (!in_array($voucher->id, $usedVoucher))
                                                                    <option value="{{ $voucher->id }}"
                                                                        ...
                                                                        {{ $voucher->vourcher_code }}
                                                                        ({{ $voucher->discount_percentage }}% giảm, tối đa
                                                                        {{ $voucher->max_discount_amount }} VNĐ)
                                                                    </option>
                                                                @endif
                                                            @endforeach
                                                        </optgroup>
                                                            ...
                                                            @foreach ($vourcherEvents as $event)
                                                                @if (!in_array($event->id, $usedVoucher) && \Carbon\Carbon::now()->greaterThanOrEqualTo($event->start_time))
                                                                    <option value="{{ $event->id }}"
                                                                        ...
                                                                        {{ $event->vourcher_code }}
                                                                        ({{ $event->discount_percentage }}% giảm, tối đa
                                                                        {{ $event->max_discount_amount }} VNĐ)
                                                                    </option>
                                                                @endif
                                                            @endforeach
                                                        </optgroup>
                                ...
                                <div class="col-md-12">
                                    <div class="st_cherity_btn float_left">
                                        ...
                                        <ul>
                                            <h3 style="color: red">Vui lòng hoàn thành trước thời gian bắt đầu ca chiếu  {{ \Carbon\Carbon::parse($showtime->shifts->start_time)->format('H:i') }} , nếu vượt quá sẽ ko thể hoàn thành mua vé ! </h3>
                                            ...
                                            @if ($selectedSeats2->isNotEmpty())
                                                ...
                                                <li><button type="submit" class="btn btn-success"
                                                        onclick="startPayment()">Thanh toán</button></a>
                                                </li>
                                            @else
                                                <li>
                            ...
                            <div class="col-md-12">
                                <div class="st_dtts_bs_wrapper float_left">
                                    ...
                                    <div class="st_dtts_sb_ul float_left">
                                        <ul id="selected-seats-list">
                                            @if ($selectedSeats2->isNotEmpty())
                                                @foreach ($selectedSeats2 as $seat)
                                                    <li>{{ $seat->seat->seat_number }}<br>
                                                        (1 Ticket)
                                                        <span>Giá: {{ number_format($seat->price) }} VNĐ</span>
                                                    </li>
                                                @endforeach
                                            @else
                                        ...
                                        {{-- <p>Combo đã chọn: <span
                                                id="selected-combo-name">{{ isset($combo) ? $combo->name : 'Không có combo nào được chọn.' }}</span>
                                        </p>
                                        <p>Giá combo: <span
                                                id="selected-combo-price">{{ isset($combo) ? number_format($combo->price) : '0 VNĐ' }}</span>
                                        </p> --}}
...
<script>
    ...
    function startPayment() {
        ...
        text: 'Sau khi chọn ghế, nếu không thanh toán sau 5 phút, ghế bạn đã chọn sẽ tự động bị bỏ. Nếu quá 3 lần thì bạn sẽ bị cấm đặt vé . Bạn có chắc chắn muốn tiếp tục?',
        icon: 'info',
        ...
        if (result.isConfirmed) {
            // Nếu người dùng nhấn "Tiếp tục", gửi form đi
            document.getElementById('payment-form').submit(); // Thay 'payment-form' bằng ID của form của bạn
        }
    });
    }
</script>
